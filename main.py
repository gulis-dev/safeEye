    ...
    while cap.isOpened():
        ...
        if frame_counter % analysis_interval == 0:
            ...
            for *xyxy, conf, cls in results.xyxy[0].tolist():
                if int(cls) == 0 and conf > 0.5:  # Wykrywamy tylko ludzi z pewnością > 0.7
                        ...
                        # Oblicz szerokość i wysokość prostokąta
                        width = x2 - x1
                        height = y2 - y1
            ...
            for track in tracks:
                ...
                if track_id not in detection_times:
                    detection_times[track_id] = datetime.now()
                ...
                if track_id not in unique_ids:
                    ...
                    # Zapis zdjęcia
                    detected_person = frame[int(max(0, bbox[1])):int(min(frame.shape[0], bbox[3])),
                        int(max(0, bbox[0])):int(min(frame.shape[1], bbox[2]))]
...
def generate_frames():
    ...
    while cap.isOpened():
        ...
        for track_id, bbox in active_tracks.items():
            ...
            # Oblicz czas od wykrycia
            if track_id in detection_times:
                elapsed_time = datetime.now() - detection_times[track_id]
                ...
                # Dodaj czas do ramki
                cv2.putText(frame, f"Time: {elapsed_time_str}", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
